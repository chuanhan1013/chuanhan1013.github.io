...
const skillGlyphMap = new Map([['Isaac Gym', 'IG'], ['Isaac Lab', 'IL']]);
...
document.querySelectorAll('.skills > div').forEach((group, i) => {
  ...
  items.forEach(label => {
    ...
    if (src) item.append(iconImg(src)); else if (skillGlyphMap.has(label)) { const glyph = document.createElement('span'); glyph.className = 'platform-glyph skill-platform-glyph'; glyph.textContent = skillGlyphMap.get(label); item.append(glyph); } else { const dot = document.createElement('span'); dot.className = 'skill-dot'; item.append(dot); }
...
[...document.querySelectorAll('#projects .feature-card')].forEach(card => {
  ...
  else if (title.includes('Bio-Inspired')) addPlatformStrip(card, [{label:'Isaac Lab',glyph:'IL'},{label:'Isaac Gym',glyph:'IG'},{label:'Python',icon:devicon('python')},{label:'PyTorch',icon:devicon('pytorch')}]);
